...
-- validates sine_calculator against the actual ECP5-targeted fixed_dsp
...
-- multiplier, wider than lut_sine_pkg's own 16-bit angle_word_length ;
-- sine_calculator resizes its operands up to whatever width fixed_dsp_in
-- actually has, so driving it with a wider dsp than its own lut tables
-- just works.
--
-- with use_gaps = false, a new angle is requested every clock cycle
...
architecture sim of sine_calculator_tb is
...
------------------------------------------------------------------------
    ...
    -- fixed_dsp lives outside sine_calculator ; instantiated here (with
    -- the ecp5 architecture, native to mpy_32x32's 32x32 width -- wider
    -- than angle_word_length) and wired straight to its ports

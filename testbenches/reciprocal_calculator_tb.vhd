...
-- validates reciprocal_calculator against the actual ECP5-targeted
-- fixed_dsp build (entity fixed_dsp, architecture ecp5, which uses the
-- mpy_32x32 hard IP -- stood in for here by sim_mpy32x32.vhd, same as
-- arch_ecp_dsp_instruction.vhd does). mpy_32x32 is a fixed 32x32
-- multiplier, wider than lut_reciprocal_pkg's own 16-bit recip_word_length ;
-- reciprocal_calculator resizes its operands up to whatever width
-- fixed_dsp_in actually has, so driving it with a wider dsp than its
-- own lut tables just works.
--
...
entity reciprocal_calculator_tb is
    generic (
        runner_cfg : string
...
architecture sim of reciprocal_calculator_tb is
...
------------------------------------------------------------------------
    ...
    -- fixed_dsp lives outside reciprocal_calculator ; instantiated here
    -- (with the ecp5 architecture, native to mpy_32x32's 32x32 width --
    -- wider than recip_word_length) and wired straight to its ports
